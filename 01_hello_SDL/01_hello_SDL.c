...
int main(int argc, char* args[])
{
	 ...
	 * SDL_CreateWindow. The first argument sets the window's caption or
	 * this part of the window:
	 * 
	 * The next two arguments define the x and y position the window is
	 * created in. Since we don't care where it is created, we just put
	 * SDL_WINDOWPOS_UNDEFINED for the x and y position.
	 * 
	 * The next two arguments define the window's width and height. The
	 * last argument are the creation flags. SDL_WINDOW_SHOWN makes sure
	 * the window is shown when it is created.
	 * 
	 * If there is an error, SDL_CreateWindow returns NULL. If there's no
	 * window, we want to print out the error to the console. 
	 */
	...
	/*
	 * If the window was created successfully, we want to get the window's
	 * surface so we can draw to it. SDL_GetWindowSurface does just this.
	 *
	 * To keep this tutorial simple, all we're going to do here is fill the
	 * window's surface white using SDL_FillRect. Don't worry too much
	 * about this function here. This tutorial is only concerned about
	 * getting a window to pop up.
	 * 
	 * An important thing to know about rendering is that just because
	 * you've drawn something to the screen surface doesn't mean you'll see
	 * it. After you've done all your drawing you need to update the window
	 * so it shows everything you drew. A call to SDL_UpdateWindowSurface
	 * will do this.
	 * 
	 * If all we do is create the window, fill it, and update it, all we're
	 * going to see is a window flash for a second and then close. To keep
	 * it from disappearing, we'll call SDL_Delay. SDL_Delay will wait for
	 * a given amount of milliseconds. A millisecond is 1/1000 of a second.
	 * This means the code above will make the window wait for 2000 1/1000
	 * of a second or 2 seconds.
	 * 
	 * An important thing to note is that when SDL is delaying, it cannot
	 * accept input from the keyboard or mouse. Don't panic when you run
	 * this program and it doesn't respond. We haven't given it the code to
	 * handle the mouse and keyboard.
	 */
	SDL_Event e;
	while(1)
	{
...
equit:
	/*
	 * After the window has delayed there for 2 seconds, we'll destroy the
	 * window to free its memory. This will also take care of the
	 * SDL_Surface we got from it. After everything is deallocated, we quit
	 * SDL and return 0 to terminate the program. 
	 */
