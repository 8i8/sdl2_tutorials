...
void LTimer_unpause(LTimer *s)
{
	if(s->mStarted && s->mPaused) {
		s->mPaused = false;
		s->mStartTicks = SDL_GetTicks() - s->mPausedTicks;
		s->mPausedTicks = 0;
	}
}
...
short LTimer_isStarted(LTimer *t)
{
	return t->mStarted;
}

short LTimer_isPaused(LTimer *t)
{
